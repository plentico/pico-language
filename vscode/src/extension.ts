...
/**
 * Interface representing an HTML element in the DOM tree
 */
 ...
 */
function parseHTMLTree(htmlContent: string): HTMLElement[] {
 ...
 */
function getAllElements(elements: HTMLElement[]): HTMLElement[] {
    const result: HTMLElement[] = [];
    
    function traverse(els: HTMLElement[]) {
        for (const el of els) {
            result.push(el);
 ...
 */
function registerCSSDiagnostics(context: vscode.ExtensionContext) {
    ...
    function updateDiagnostics(document: vscode.TextDocument) {
        ...
        const cssBlocks: Array<{ content: string; startLine: number }> = [];
        let styleStart = -1;
        let styleContent = '';
                ...
                if (selectorMatch) {
                    ...
                    for (const selector of selectors) {
                        ...
                        if (selectorParts.length === 1) {
                            ...
                            if (!found) {
                                createDiagnostic(diagnostics, line, lineNumber, selectorPart, 
                            ...
                            if (unmatchedPart) {
                                createDiagnostic(diagnostics, line, lineNumber, unmatchedPart.part,
                                    `CSS selector '${unmatchedPart.part}' does not match any ${unmatchedPart.type} in this component's structure`);
                            }
                        }
                    }
                }
     ...
     */
    function buildParentMap(elements: HTMLElement[]): Map<HTMLElement, HTMLElement | null> {
        const map = new Map<HTMLElement, HTMLElement | null>();
        
        function traverse(els: HTMLElement[], parent: HTMLElement | null) {
            for (const el of els) {
                map.set(el, parent);
     ...
     */
    function hasMatchingAncestors(
        element: HTMLElement,
        ancestorParts: string[],
        parentMap: Map<HTMLElement, HTMLElement | null>
    ): boolean {
     ...
     */
    function findMissingAncestorChain(
        selectorParts: string[],
     ...
     */
    function hasDescendantMatching(element: HTMLElement, selectorPart: string): boolean {
        function traverse(els: HTMLElement[]): boolean {
            for (const el of els) {
